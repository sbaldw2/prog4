...
public class Semant {
	...
	public void transProg(Absyn.Exp exp) {
		...
		// ALLOCATE NEW LEVEL FOR TRANSLATION OF BODY, BEFORE CALL
		// TRANSEXP
	...
	ExpTy transExp(Absyn.ForExp e) {
		// ALLOCATE VAR, BEFORE CONSTRUCTING ENTRY WITH RESULTS
	...
	Exp transDec(Absyn.TypeDec d) {
		// 1ST PASS - HANDLES TYPE HEADERS
		// USES LOCAL HASHTABLE TO CHECK IF THERE ARE TWO TYPES
		// WITH SAME NAME IN SAME (CONSECTUTIVE) BATCH
		// OF MUTALLY RECURSIVE TYPES. SEE TEST38.TIG!
	...
	Exp transDec(Absyn.FunctionDec d) {
		// 1ST PASS - HANDLES FUNCTION HEADERS
		Hashtable hash = new Hashtable();
		for (Absyn.FunctionDec f = d; f != null; f = f.next) {
			// CONSTRUCT A NEW LEVEL FOR THE FUNC WITH INFO ON WHICH PARAMS TO THE FUNC ESC
			// BEFORE CONSTRUCTING THE FUNC ENTRY WITH NEW LEVEL
			// THE FUNC BODY NEEDS TO BE TRANSLATED IN THIS NEW LEVEL
		...
		// 2ND PASS - HANDLES FUNCTION BODIES
		for (Absyn.FunctionDec f = d; f != null; f = f.next) {
			env.venv.beginScope();
			// F.entry.level.formals
